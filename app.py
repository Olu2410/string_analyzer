...
# Defines the path to the storage file in the same directory as main.py
...
class StringRequest(BaseModel):
    """Defines the expected structure for POST /strings request body."""
    value: str
    ...
    if STORAGE_PATH.exists():
        try:
            with open(STORAGE_PATH, 'r') as f:
                # Use str keys to match storage dict definition
                return json.load(f)
...
def save_data_to_file():
    ...
    # Note: This is synchronous, which is fine for small files and local use.
...
def analyze_string(text: str) -> Dict[str, Any]:
    ...
    # Hash of the cleaned text to ensure consistency
...
async def create_analyze_string(request_data: StringRequest): # Accepts Pydantic model
    
    value = request_data.value # Access value via model instance

    if not value.strip():
        # Handles strings consisting only of whitespace
        raise HTTPException(
    ...
    if value in storage:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, # Correct status for duplicate
...
async def get_all_strings(
    ...
    # max_length=1 ensures Pydantic validation handles non-single character input (422)
    contains_character: Optional[str] = Query(None, max_length=1) 
):
    # Custom validation for length conflict
    if min_length and max_length and min_length > max_length:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail="Invalid query parameter values or types" # Detail matches required error message
        )
    
    # Build filters dictionary for the response
    filters = {
...
async def filter_by_natural_language(query: str):
    ...
    if "contain" in q:
        # Tries to find "letter X" where X is a word character
        m = re.search(r"letter (\w)", q)
        if m:
            parsed_filters["contains_character"] = m.group(1)
            
    # simple heuristic for "vowel"
